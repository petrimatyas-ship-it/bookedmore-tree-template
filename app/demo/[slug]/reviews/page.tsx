...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!demo) return {};
  return {
    title: `Reviews | ${demo.config.companyName}`,
    description: `Tree service reviews from homeowners across ${demo.config.city}.`
  };
}

export default async function DemoReviewsPage({ params }: Params) {
  const { slug } = await params;
  const demo = await loadDemo(slug);
  if (!demo) return null;
  ...
  return (
    <>
      <PageHead
        ...
        description="The work matters, but so does everything around it: showing up on time, communicating clearly, and leaving the yard spotless."
      />
      ...
      <section className="px-5 py-16 lg:px-8">
        <div className="mx-auto max-w-6xl">
          {config.reviewSummary ? (
            ...
            <div className="rounded-[22px] border border-dashed border-forest-900/20 bg-white p-8 text-center shadow-soft">
              <p className="text-lg font-bold text-forest-900">{demoCopy.reviews.emptyTitle}</p>
              <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-forest-900/70">
