...
export const metadata = {
  title: `Pricing | ${business.companyName}`,
  description: `Real tree service price ranges in ${business.city} by tree size, with recent example jobs and what moves the number.`
};
...
export default function PricingPage() {
  return (
    <PageShell
      ...
      description={`Nobody publishes real numbers, so estimates feel like a lottery. Here are the honest ranges we quote in ${business.city}, plus recent jobs with the actual price we charged.`}
    >
      ...
      <section className="px-5 py-16 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {business.priceGuide.map((guide) => (
              <div key={guide.service} className="flex flex-col overflow-hidden rounded-[18px] border border-forest-900/10 bg-white shadow-soft">
                <div className="border-b border-forest-900/10 bg-forest-900 px-6 py-4">
                  <h2 className="text-base font-bold text-white">{guide.service}</h2>
                </div>
          ...
          <p className="mt-4 text-sm text-forest-900/60">
            Every estimate is free and in writing. The price we quote is the price you pay. No fuel surcharges and no surprise cleanup fees. Emergency
            hazards get secured first, paperwork comes after.
          </p>
        </div>
      </section>

      {/* Recent jobs with real prices */}
      <section className="px-5 pb-16 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-3 max-w-2xl text-base leading-7 text-forest-900/70">
            Pulled from this year&apos;s work orders so you can calibrate what your job might run.
          </p>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            {business.exampleJobs.map((job) => (
              <article key={job.title} className="flex h-full flex-col overflow-hidden rounded-[18px] border border-forest-900/10 bg-white shadow-soft">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {costFactors.map((factor, i) => {
              ...
              return (
              <article key={factor.title} className="rounded-[18px] border border-forest-900/10 bg-white p-6 shadow-soft">
          ...
          <div className="mt-6 rounded-[18px] border border-forest-900/10 bg-forest-50 p-6">
            <h3 className="text-base font-bold text-forest-900">Payment, simply</h3>
            <p className="mt-2 text-sm leading-7 text-forest-900/72">
              No deposits on standard residential jobs. You pay by card, check, or bank transfer after the work is done
              and you&apos;ve walked the yard with the crew lead. Insurance-claim storm work? We document everything and
              provide itemized invoices your adjuster will accept.
            </p>
          </div>

          <h2 className="mt-14 text-2xl font-bold text-forest-900 sm:text-3xl">Ways to keep the price down</h2>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {business.savingsTips.map((tip, i) => {
              const TipIcon = SAVINGS_ICONS[i % SAVINGS_ICONS.length];
              return (
              <article key={tip.title} className="rounded-[18px] border border-forest-900/10 bg-white p-6 shadow-soft">
                <h3 className="flex items-center gap-2.5 text-base font-bold text-forest-900">
          ...
          <h2 className="mt-14 text-2xl font-bold text-forest-900 sm:text-3xl">Pricing questions we get a lot</h2>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {business.pricingFaqs.map((faq) => (
              <article key={faq.question} className="rounded-[18px] border border-forest-900/10 bg-white p-6 shadow-soft">
                <h3 className="flex items-start gap-2.5 text-base font-bold leading-snug text-forest-900">
                  <IconHelpCircle size={20} stroke={1.9} className="mt-0.5 shrink-0 text-forest-600" aria-hidden="true" />
                  {faq.question}
                </h3>
