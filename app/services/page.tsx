...
export const metadata = {
  ...
  description: `Tree removal, trimming, stump grinding, emergency service, and more across ${business.city}.`
};
...
export default function ServicesPage() {
  return (
    <PageShell
      ...
      description={`From single-branch trims to crane-assisted removals, our ${business.city} crews show up with the right equipment and leave the yard clean. Here's what each service actually involves.`}
    >
      {/* Emergency banner */}
      <section className="px-5 pt-10 lg:px-8">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-4 rounded-[18px] border-2 border-ember-500/40 bg-ember-500/8 p-6 sm:flex-row sm:items-center">
          <div>
            <p className="text-sm font-extrabold uppercase tracking-[0.14em] text-ember-600">Storm damage right now?</p>
            <p className="mt-1 text-sm leading-6 text-forest-900/75">
              Tree on the house, hanging limb, or blocked driveway? Skip the form and call. Emergency calls are dispatched same-day, 24/7.
            </p>
          </div>
          <a
      ...
      <section className="px-5 py-14 lg:px-8">
        <div className="mx-auto grid max-w-6xl gap-10">
          {business.serviceCards.map((service, index) => (
            <Reveal key={service.title}>
            <article
              className="grid overflow-hidden rounded-[22px] border border-forest-900/10 bg-white shadow-soft lg:grid-cols-[0.9fr_1.1fr]"
            >
              ...
              <div className="flex flex-col p-7 sm:p-9">
                ...
                <div className="mt-6 grid gap-5 sm:grid-cols-2">
                    ...
                    <div className="mt-3 grid gap-2.5">
                      {service.signs.map((sign) => (
                        <span key={sign} className="flex gap-2.5 text-sm leading-6 text-forest-900/72">
                          <span className="font-bold text-ember-500" aria-hidden="true">›</span>
                          <span>{sign}</span>
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-forest-700">What&apos;s included</p>
                    <div className="mt-3 grid gap-2.5">
                      {service.included.map((item) => (
                        <span key={item} className="flex gap-2.5 text-sm leading-6 text-forest-900/72">
                          <span className="font-bold text-forest-700" aria-hidden="true">✓</span>
                          <span>{item}</span>
                        </span>
                      ))}
                    </div>
