  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:hidden">
        <div className="mx-auto grid max-w-md grid-cols-[1fr_1.4fr] gap-2 rounded-full bg-ink p-1.5 shadow-[0_18px_40px_rgba(20,23,26,0.35)]">
          {config.phone ? (
            <a
              href={`tel:${config.phone}`}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-full text-[15px] font-bold text-bone"
            >
          ...
          <a
            ...
            className="inline-flex h-12 items-center justify-center gap-1.5 rounded-full bg-signal text-[15px] font-bold text-ink"
          >
            Free estimate
